...
// This file contains a lot of method names that have coordinates, such as SetXY, so the normal naming rule is not used. 
...
namespace RTCube.Extensions
{
	...
	public static class VectorExtensions
	{
		...
		/// 	<para>The perp dot product can be calculated using this: <c>var perpDotProduct = Vector2.Dot(v1.Perp(), v2);</c></para>
		/// </remarks>
		...
		{
			return new Vector2(
				RTCMathf.FloorMod(thisVector.x, otherVector.x), 
				RTCMathf.FloorMod(thisVector.y, otherVector.y));
		}
		...
		public static Vector3 HadamardMod(this Vector3 thisVector, Vector3 otherVector)
		{
			return new Vector3(
				RTCMathf.FloorMod(thisVector.x, otherVector.x), 
				RTCMathf.FloorMod(thisVector.y, otherVector.y),
				RTCMathf.FloorMod(thisVector.z, otherVector.z));
		}
		...
		public static Vector4 HadamardMod(this Vector4 thisVector, Vector4 otherVector)
		{
			return new Vector4(
				...
				RTCMathf.FloorMod(thisVector.y, otherVector.y),
				RTCMathf.FloorMod(thisVector.z, otherVector.z),
				RTCMathf.FloorMod(thisVector.w, otherVector.w));
		}
		...
		/// <remarks>
		/// Unity provides `ClampMagnitude` for Vector2, and Vector3: <see cref="Vector2.ClampMagnitude"/>
		/// and <see cref="Vector3.ClampMagnitude"/>. This method provides the same functionality for Vector4.
		/// </remarks>
