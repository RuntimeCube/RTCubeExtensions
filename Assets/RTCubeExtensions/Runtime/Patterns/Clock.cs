...
namespace RTCube.Extensions
{
	...
	public class Clock
	{
		...
		private float time;
		private readonly ObservedValue<int> timeInSeconds;
		...
		public Clock()
		{
			...
			timeInSeconds.OnValueChange += NotifySecondsChange;
		...
		public void Reset(float startTime)
		{
			if (startTime < 0)
			{
				throw new ArgumentException("Must be non-negative", nameof(startTime));
			}
		...
		private void NotifySecondsChange() => OnSecondsChanged?.Invoke();
