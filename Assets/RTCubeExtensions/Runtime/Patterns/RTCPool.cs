...
namespace RTCube.Extensions	
{
	...
	/* Design note: we _could_ enforce T to implement a certain interface, and provide and maintain the isActive field
		...
		A better name for this class would be `DefaultPool` so we could use RTCPool for static methods (including factory 
		methods that could give you different pools more conveniently). Since this is a very old class though, I did not
		want to rename it. :/
	*/
	public class RTCPool<T> : IPool<T>
		where T : class
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="RTCPool{T}"/> class.
		/// </summary>
