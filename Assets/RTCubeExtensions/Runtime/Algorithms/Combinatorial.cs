	...
	public static class Combinatorial
	{
		...
		private static IEnumerable<T> PermutationsImpl<T>(int n, Func<int[], T> select)
		{
			...
			while (true)
			{
				...
				RTCDebug.Assert(index >= 0, "index must be greater than or equal to 0");
				RTCDebug.Assert(index + 1 < permutation.Length, "index + 1 must be smaller than permutation length, but index == " + index);
