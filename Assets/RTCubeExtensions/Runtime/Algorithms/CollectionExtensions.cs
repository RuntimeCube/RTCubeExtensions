...
using RTCube.Extensions.Support;
...
namespace RTCube.Extensions.Algorithms
{
	...
	public static class CollectionExtensions
	{
		...
		private class CountableEnumerable<T> : IReadOnlyCollection<T>
		{
			...
			[MustDisposeResource] // ReSharper disable once NotDisposedResourceIsReturned
		...
		{
			return RandomItem(source, RTCRandom.GlobalRandom);
		}
		...
		{
			return SampleRandom(source, sampleCount, RTCRandom.GlobalRandom);
		}
		...
		{
			list.Shuffle(RTCRandom.GlobalRandom);
		}
		...
		/// enumerates multiple times, so if you want to avoid this e numerate the collection to something else.
		/// </remarks>
		[Version(1, 0, 0)]
